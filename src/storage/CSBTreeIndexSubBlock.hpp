...
class CompositeKeyLessComparator;
...
class CSBTreeIndexSubBlock : public IndexSubBlock {
 public:
  ...
  IndexSubBlockType getIndexSubBlockType() const {
    return kCSBTree;
  }

  bool supportsAdHocAdd() const {
    return initialized_;
  }

  bool supportsAdHocRemove() const {
    return true;
  }

  bool addEntry(const tuple_id tuple);

  void removeEntry(const tuple_id tuple);

  /**
   * @note Currently this version only supports simple comparisons of a literal
   *       value with a non-composite key.
   **/
  IndexSearchResult getMatchesForPredicate(const Predicate &predicate) const;
 ...
 private:
  ...
  void* findLeaf(const void *node, const void *key) const;

  // Specialized version of findLeaf() which uses 'literal_less_key_comparator'
  // and 'key_less_literal_comparator' to compare '*literal' with keys in
  // '*node'. This is intended for searches where '*literal' is not the same
  // exact type as the keys, but it comparable to them.
  void* findLeafWithComparators(const void *node,
                                const void *literal,
  ...
  void insertEntryInLeaf(const tuple_id tuple, const void *key, void *node);
  ...
  void removeEntryFromLeaf(const tuple_id tuple, const void *key, void *node);
  ...
  // entry for every tuple in 'tuple_store_' with a non-NULL key. This version
  // is for the case where the key is non-composite and
  // tuple_store_.supportsUntypedGetAttributeValue(key) is true.
  void generateEntryReferencesFromUntypedPtrs(
      std::vector<csbtree_internal::EntryReference> *entry_references) const;

  // Helper method for rebuildLeaves(). Fill in '*entry_references' with an
  // entry for every tuple in 'tuple_store_' with a non-NULL key. This version
  // is for the case where the key is non-composite and
  // tuple_store_.supportsUntypedGetAttributeValue(key) is false. The
  // LiteralTypeInstances created by this method are added to
  // '*literal_typed_keys'.
  void generateEntryReferencesFromTypeInstances(
      std::vector<csbtree_internal::EntryReference> *entry_references,
      PtrVector<TypeInstance, false> *literal_typed_keys) const;

  // Helper method for rebuildLeaves(). Fill in '*entry_references' with an
  // entry for every tuple in 'tuple_store_' with a non-NULL key. This version
  // is for the case where the key is non-composite and compressed.
  ...
  // Helper method for rebuildLeaves(). Fill in '*entry_references' with an
  // entry for every tuple in 'tuple_store_' with a non-NULL key. This version
  // is for the case where the key is composite. The key copies created by this
  // method are added to '*composite_key_buffers'.
  ...
  bool initialized_;
  bool tuple_store_supports_untyped_ptr_;
  ...
  ScopedPtr<UncheckedComparator> key_comparator_;
  ...
  ScopedPtr<BitVector> node_group_used_bitmap_;
